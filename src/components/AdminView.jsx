// ============================================================================
// AdminView.jsx - Redesigned Administrator Dashboard View
// ============================================================================
// INTEGRATED MODERN DESIGN with existing functionality
// ============================================================================
...
// ============================================================================
// ALERT MONITORING HOOK (Unchanged)
// ============================================================================
const useAlertMonitoring = (alerts, trafficData, blockedAttempts, systemStats) => {
  ...
  useEffect(() => {
    ...
    if (analysis.requiresPopup) {
      setCriticalAlert(formattedAlert);
      playAlertSound();
    } else {
  ...
  const handleEscalate = () => {
    if (criticalAlert) {
      alert('Alert has been escalated to the incident response team.');
      setCriticalAlert(null);
    }
  };
...
const AdminView = ({
  ...
  const handleGenerateReport = (period) => {
    if (period === 'custom') {
      generateFalsePositiveReport('custom', reportStartDate, reportEndDate);
    } else {
      generateFalsePositiveReport(period);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ====================================================================== */}
      {/* SYSTEM OVERVIEW - Modern Stats Cards */}
      {/* ====================================================================== */}
      <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
          ...
          <h3 className="text-xl font-bold text-white">Analytics & Monitoring</h3>
        </div>

        {/* Network Traffic Flow - Full Width */}
        <div style={{ marginBottom: '2rem' , height:520}}>
    ...
    <div className="space-y-6">
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
          ...
          <h3 className="text-xl font-bold text-white">Network Performance Monitor</h3>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(500px,1fr))', gap: '2rem' }}>
          ...
          <div>
            ...
            {/* Summary */}
            {networkUsageData.length > 0 && (() => {
              const latest = networkUsageData[networkUsageData.length - 1];
              return (
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))', gap:'0.75rem', marginTop:'1rem' }}>
                  {[
                    { label:'Users', value:latest.activeUsers, color:'white' },
                    { label:'Upload', value:`${latest.uploadSpeed} Mbps`, color:'#06b6d4' },
                    { label:'Download', value:`${latest.downloadSpeed} Mbps`, color:'#3b82f6' },
                    { label:'Usage', value:`${latest.totalUsage} MB`, color:'#10b981' }
                  ].map((item, idx) => (
                    <div key={idx} style={{
                      textAlign:'center',
                      padding:'0.75rem',
                      ...
                      <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'0.75rem', marginBottom:'0.25rem' }}>{item.label}</p>
                      <p style={{ color:item.color, fontWeight:'700', fontSize:'0.9rem' }}>{item.value}</p>
                    </div>
                  ))}
                </div>
              );
          ...
          <div>
            ...
            <div style={{ maxHeight: '280px', overflowY: 'auto' }}>
              {userLatencies.slice(0, 5).map((user, idx) => (
                <div key={idx} style={{
                  background: 'rgba(255, 255, 255, 0.05)',
                  border: '1px solid rgba(255, 255, 255, 0.1)',
                  borderRadius: '12px',
                  ...
                  <div>
                    <p style={{ color: 'white', fontWeight: '600', fontSize: '0.95rem' }}>{user.username}</p>
                    <p style={{ color: 'rgba(255, 255, 255, 0.5)', fontSize: '0.8rem' }}>{user.role}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div style={{ textAlign: 'center' }}>
                      <p style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '0.75rem', marginBottom: '0.25rem' }}>Latency</p>
                      <p style={{ 
                        color: user.quality === 'low' ? '#10b981' : user.quality === 'medium' ? '#3b82f6' : '#f59e0b',
                        fontWeight: '700',
                        fontSize: '0.9rem'
                      }}>
                        {user.latency}ms
                    ...
                    <span style={{
                      ...
                      fontSize: '0.75rem',
                      fontWeight: '600',
                      background: user.quality === 'low' ? 'rgba(16, 185, 129, 0.2)' : user.quality === 'medium' ? 'rgba(59, 130, 246, 0.2)' : 'rgba(245, 158, 11, 0.2)',
                      color: user.quality === 'low' ? '#10b981' : user.quality === 'medium' ? '#3b82f6' : '#f59e0b'
                    }}>
                      {user.status}
                    </span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', marginTop: '1rem' }}>
              {[
                { label: 'Low Latency', value: userLatencies.filter(u => u.quality === 'low').length, color: '#10b981' },
                { label: 'Medium', value: userLatencies.filter(u => u.quality === 'medium').length, color: '#3b82f6' },
                { label: 'High Latency', value: userLatencies.filter(u => u.quality === 'high').length, color: '#f59e0b' }
              ].map((item, idx) => (
                <div key={idx} style={{ textAlign: 'center', padding: '0.75rem', borderRadius: '12px', background: 'rgba(255,255,255,0.05)' }}>
                  <p style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '0.75rem', marginBottom: '0.25rem' }}>{item.label}</p>
                  <p style={{ color: item.color, fontWeight: '700', fontSize: '0.9rem' }}>{item.value}</p>
                </div>
              ))}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ color: 'rgba(255, 255, 255, 0.6)', marginBottom: '2rem', fontSize: '0.95rem' }}>
              Export detailed security analysis reports for various time periods
            </p>
            ...
            <div style={{
              ...
              <p style={{ color: 'rgba(255, 255, 255, 0.8)', fontSize: '0.875rem' }}>
                <strong style={{ color: 'white' }}>Note:</strong> Reports include false positive analysis, 
                security alerts, blocked attempts, and traffic patterns for the selected period.
              </p>
            </div>
          </div>
        </div>
